...
export const editFile = ({ session, dataStream }: EditFileProps) =>
  tool({
    description:
      "Make a targeted edit to an existing file by finding and replacing an exact string. Preferred over writeFile for targeted changes. The old_string must match exactly.",
    execute: async ({ id, old_string, new_string, replace_all }) => {
      ...
      if (!document.content) {
        return { error: "File has no content" };
      }
    ...
    inputSchema: z.object({
      id: z.string().describe("The ID of the file to edit"),
      new_string: z
        .string()
        .describe(
...
export const editDocument = editFile;
